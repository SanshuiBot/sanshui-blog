...
const skills = [
  { label: 'Next.js', level: 90 }, { label: 'React', level: 92 },
  { label: 'TypeScript', level: 85 }, { label: 'Tailwind CSS', level: 95 },
  { label: 'Node.js', level: 80 }, { label: 'Python', level: 70 },
];
const stack = [
    ...
    <div className="relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-24">
      <ScrollReveal direction="up" delay={0.05}>
        <Link href="/" className="inline-flex items-center gap-1.5 text-sm text-stone-500 hover:text-stone-900 dark:hover:text-stone-200 transition-colors mb-8 group/back">
          <ArrowLeft size={16} className="transition-transform duration-300 group-hover/back:-translate-x-1" />返回首页
        </Link>
      </ScrollReveal>
      <ScrollReveal direction="up" delay={0.1}>
        <div className="relative mb-12">
          ...
          <span className="inline-flex items-center gap-1.5 text-xs font-medium text-red-600 dark:text-red-400 uppercase tracking-widest mb-4"><Sparkles size={12} />关于</span>
          <h1 style={{ viewTransitionName: 'page-title' }} className="text-4xl sm:text-5xl font-bold text-stone-900 dark:text-stone-50 tracking-tight text-balance">
      ...
      <ScrollReveal direction="up" delay={0.15}>
        <div className="prose-custom space-y-6 relative">
          ...
          <p>一个热爱技术和写作的人. 我相信写作是最好的学习方式——通过输出可以加深对知识的理解和记忆，也能遇见志同道合的朋友.</p>
          <p>这个博客是我记录学习过程、分享技术心得和生活感悟的地方. 所有内容均用 Next.js + MDX 构建，托管在 GitHub Pages 上.</p>
        </div>
      </ScrollReveal>
      <ScrollReveal direction="up" delay={0.2}>
        <div className="mt-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {skills.map((skill) => (
              <div key={skill.label}>
                <div className="flex justify-between text-sm mb-2"><span className="font-medium text-stone-700 dark:text-stone-300">{skill.label}</span><span className="text-stone-400">{skill.level}%</span></div>
                <div className="h-2 rounded-full bg-stone-100 dark:bg-stone-800 overflow-hidden">
                  <div className="h-full rounded-full bg-gradient-to-r from-pink-500 via-violet-500 to-cyan-500" style={{ width: `${skill.level}%`, transition: 'width 0.8s var(--ease-out-expo)' }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <ScrollReveal direction="up" delay={0.25}>
        <div className="mt-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {stack.map((s) => (
              <div key={s.title} className="group/stack relative p-5 rounded-2xl bg-white/80 dark:bg-stone-900/80 backdrop-blur border border-stone-200/60 dark:border-stone-800/60 hover:shadow-iridescent transition-all duration-700 overflow-hidden">
                ...
                <h3 className="text-sm font-semibold text-stone-900 dark:text-stone-50 mb-3 uppercase tracking-widest">{s.title}</h3>
      ...
      <ScrollReveal direction="up" delay={0.35}>
        <div className="mt-16 p-8 rounded-3xl double-bezel-outer relative">
          ...
          <div className="relative">
            ...
            <p className="text-stone-600 dark:text-stone-400 mb-6 text-pretty">如果你有任何问题或想法，欢迎通过以下方式联系我.</p>
            <div className="flex flex-wrap gap-3">
              <a href="https://github.com/SanshuiBot" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-stone-900 dark:bg-stone-50 text-white dark:text-stone-900 text-sm font-medium hover:scale-105 active:scale-95 transition-transform light-ray"><Code2 size={16} />GitHub</a>
              <a href="mailto:localhost6@foxmail.com" className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white dark:bg-stone-900 text-stone-700 dark:text-stone-300 text-sm font-medium border border-stone-200 dark:border-stone-800 hover:scale-105 active:scale-95 transition-transform light-ray"><Mail size={16} />Email</a>
              <a href="#" className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white dark:bg-stone-900 text-stone-700 dark:text-stone-300 text-sm font-medium border border-stone-200 dark:border-stone-800 hover:scale-105 active:scale-95 transition-transform hover:border-pink-300 dark:hover:border-pink-700 hover:shadow-iridescent"><Heart size={16} className="text-pink-500" />赞助我</a>
            </div>
          </div>
        </div>
      </ScrollReveal>
