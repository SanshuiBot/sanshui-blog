...
interface GradientTextProps {
  children: ReactNode; className?: string; mode?: 'holo' | 'rainbow' | 'sunset' | 'aurora' | 'neon' | 'ice' | 'fire';
  size?: 'sm' | 'md' | 'lg' | 'xl' | '2xl'; animated?: boolean; glow?: boolean;
}
...
const SIZES: Record<string, string> = { sm: 'text-lg sm:text-xl', md: 'text-2xl sm:text-3xl', lg: 'text-3xl sm:text-4xl lg:text-5xl', xl: 'text-4xl sm:text-5xl lg:text-7xl', '2xl': 'text-5xl sm:text-6xl lg:text-8xl' };

export default function GradientText({ children, className = '', mode = 'holo', size = 'md', animated, glow = false }: GradientTextProps) {
  const isAnimated = animated ?? mode !== 'sunset';
  return (
    <span className={`${SIZES[size]} font-bold tracking-tight ${className}`} style={{
      background: MODES[mode], backgroundSize: isAnimated ? '200% 100%' : '100% 100%',
      ...
      animation: isAnimated ? 'holo-shift 6s linear infinite' : 'none',
    }}>
      {children}
    </span>
  );
