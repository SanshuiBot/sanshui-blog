...
export default function PostCard({ post, index }: { post: any; index: number }) {
  ...
  return (
    <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} transition={{delay:index*.06,duration:.7}}>
      <motion.div ref={ref} onMouseMove={onMove} onMouseLeave={onLeave} style={{rotateX:srx,rotateY:sry,transformStyle:'preserve-3d',perspective:'1000px'}} className="group relative">
        ...
        <div className="p-[1.5px] rounded-3xl bg-gradient-to-b from-stone-200/60 via-violet-200/30 to-transparent dark:from-stone-800/40 dark:via-violet-900/20 dark:to-transparent transition-all duration-700 group-hover:from-fuchsia-500/40 group-hover:via-violet-500/30 group-hover:to-cyan-500/20">
          <article className="relative flex flex-col bg-white/90 dark:bg-stone-900/90 backdrop-blur-md rounded-3xl overflow-hidden transition-all duration-700 group-hover:shadow-[0_0_30px_rgba(244,114,182,0.2)] group-hover:-translate-y-1">
            <div className="h-[2px] bg-gradient-to-r from-transparent via-transparent to-transparent group-hover:from-fuchsia-500 group-hover:via-violet-500 group-hover:to-cyan-500 transition-all duration-1000 group-hover:h-[3px]"/>
            ...
            <div className="flex-1 p-6 sm:p-7 relative">
              ...
              <h2 className="text-xl sm:text-2xl font-bold text-stone-900 dark:text-stone-50 mb-3 group-hover:text-fuchsia-600 dark:group-hover:text-fuchsia-400 transition-colors line-clamp-2 tracking-tight"><Link href={`/posts/${post.slug}`} className="after:absolute after:inset-0">{post.title}</Link></h2>
